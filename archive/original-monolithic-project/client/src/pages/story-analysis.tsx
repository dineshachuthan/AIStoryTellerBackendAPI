...
export default function StoryAnalysis() {
  ...
  const handlePlayEmotionSample = async (emotion: string, intensity: number) => {
    ...
    try {
      ...
      const result = await apiClient.emotions.generateSample({
        emotion,
        intensity,
  ...
  const generateComprehensiveAnalysis = async (story: any) => {
    ...
    try {
      ...
      try {
        narrativeResponse = await apiClient.stories.getNarrative(parseInt(storyId));
        console.log('Found existing narrative analysis');
      } catch (fetchError: any) {
        if (fetchError.message?.includes('404')) {
          console.log('No existing narrative analysis, generating new one...');
          narrativeResponse = await apiClient.stories.createNarrative(parseInt(storyId));
        } else {
          throw fetchError;
        }
      }
      ...
      // Title is automatically updated by backend during narrative analysis - no frontend update needed
      ...
      try {
        ...
        const rolePlayResponse = await apiClient.stories.getRoleplay(parseInt(storyId));
