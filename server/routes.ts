...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Manual Voice Cloning Endpoint
  app.post('/api/voice-cloning/manual-clone', requireAuth, async (req, res) => {
    try {
      const userId = (req.user as any)?.id;
      if (!userId) {
        return res.status(401).json({ message: 'User not authenticated' });
      ...
      const { storyId, voiceName, voiceType, intensity, context, quote } = req.body;
      
      if (!storyId || !voiceName || !voiceType) {
        return res.status(400).json({ message: 'Missing required fields: storyId, voiceName, voiceType' });
      }

      // Check if user has a voice recording for this voice
      const userVoiceEmotions = await storage.getUserVoiceEmotions(userId);
      const hasRecording = userVoiceEmotions.some((emotion: any) => 
        emotion.emotion.toLowerCase() === voiceName.toLowerCase()
      );

      if (!hasRecording) {
        return res.status(400).json({ 
          message: 'No voice recording found. Please record a voice sample first.' 
        });
      }

      // Create or update voice requirement for this story
      const existingRequirement = await storage.getStoryVoiceRequirement(storyId, voiceName, voiceType);
      
      if (existingRequirement) {
        // Update existing requirement
        await storage.updateStoryVoiceRequirement(existingRequirement.id, {
        ...
        await storage.createStoryVoiceRequirement({
          storyId,
          voiceName,
          voiceType,
          recordingStatus: 'completed',
      ...
      // Start the actual voice cloning process
      const { voiceTrainingService } = await import('./voice-training-service');
      setTimeout(async () => {
        try {
          await voiceTrainingService.triggerManualTraining(userId, voiceName, voiceType, {
            storyId,
            context,
            quote,
            intensity
          ...
          console.error('Background voice cloning failed:', error);
          // Update status to failed
          const requirement = await storage.getStoryVoiceRequirement(storyId, voiceName, voiceType);
          if (requirement) {
            await storage.updateStoryVoiceRequirement(requirement.id, {
              cloneStatus: 'failed'
            });
          }
        }
      ...
      console.error('Error starting manual voice cloning:', error);
      res.status(500).json({ message: 'Failed to start voice cloning' });
    }
  });

  // Get voice cloning status
  app.get('/api/voice-cloning/status', requireAuth, async (req, res) => {
    try {
      const userId = (req.user as any)?.id;
      if (!userId) {
        return res.status(401).json({ message: 'User not authenticated' });
      ...
      const userVoiceClones = await storage.getUserVoiceClones(userId);
      const cloneStatus = userVoiceClones.reduce((acc: any, clone: any) => {
        const key = `${clone.voiceName}_${clone.voiceType}`;
        acc[key] = clone.trainingStatus;
        return acc;
      }, {});

      res.json({ status: cloneStatus });
    } catch (error: any) {
      console.error('Error fetching voice cloning status:', error);
      res.status(500).json({ message: 'Failed to fetch cloning status' });
    }
  });

  // Get story voice requirements
  app.get('/api/stories/:storyId/voice-requirements', requireAuth, async (req, res) => {
    try {
      const { storyId } = req.params;
      const requirements = await storage.getStoryVoiceRequirements(parseInt(storyId));
      res.json(requirements);
    } catch (error: any) {
      console.error('Error fetching story voice requirements:', error);
      res.status(500).json({ message: 'Failed to fetch voice requirements' });
    }
  });
